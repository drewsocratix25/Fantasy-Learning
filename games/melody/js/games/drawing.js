...
(function () {
  ...
  const pick = {
    ...
    enter() { this.t = 0; this.layout(); this.loadThumbs(); FL.Audio.say(LINES().welcome); },
    ...
    layout() {
      ...
      list.forEach((d, i) => { const r = Math.floor(i / cols), c = i % cols; this.cards.push(new UI.Button({ x: x0 + c * (cw + gap), y: y0 + r * (ch + gap), w: cw, h: ch, color: d.color, pic: d, onTap: () => { FL.Audio.say(LINES().pick(d)); G().go('drawing', { id: d.id }); } })); });
    },
    resize() { this.layout(); },
  ...
  const lesson = {
    ...
    enter(params) {
      ...
      this.steps = this.pic.steps.concat(this.pic.fills ? [{ say: LINES().colour, strokes: [], color: true }] : []);
    ...
    startStep(first, goingBack) {
      ...
      const praise = !first && !goingBack && this.drewOn.has(this.step - 1); const PR = LINES().praise;
      if (praise) FL.Audio.say(PR[Math.floor(Math.random() * PR.length)]);
      FL.Audio.say(st.say, { interrupt: !praise });
      if (first && this.helper) FL.Audio.say(LINES().hint, { interrupt: false });
    },
    ...
    update(dt) {
      ...
      if (!this.anim && !st.color && !this.remind && this.stepT > 16 && !this.drewOn.has(this.step)) { this.remind = true; FL.Audio.say(LINES().remind); }
    },
  ...
  lesson.music = 'garden';
  lesson.cancel = function (p) { if (this.cur && this.cur.id === p.id) { this.cur = null; this.rerender(); } };
