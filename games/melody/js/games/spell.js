...
(function () {
  ...
  const scene = {
    ...
    listen() {
      ...
      rec.onresult = (e) => {
        if (this.rec !== rec || G().sceneName !== 'spell') return;
        let finalT = '', interim = '';
      ...
      rec.onerror = (e) => { if (this.rec !== rec) return; this.recError = e && e.error; };
      rec.onend = () => {
    ...
    submitTyped() { this.stopListening(); const input = this.input(); const v = input.value; input.blur(); if (!v.trim()) { this.focusInput(); return; } this.ask(v); },
    // ---- input ------------------------------------------------------------------------------------------
    down(p) {
